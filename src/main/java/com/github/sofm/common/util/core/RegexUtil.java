...
public class RegexUtil {
  ...
  private static final Pattern DEFAULT_VALID_PHONE_NUMBER =
      Pattern.compile("^\\+(?:[0-9]?){6,14}[0-9]$");

  private static final Pattern DEFAULT_VALID_IPV4_REGEX =
      Pattern.compile(
          "^((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\.){3}(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$");

  private static final Pattern DEFAULT_VALID_IPV6_REGEX =
      Pattern.compile(
  ...
  public static boolean isValidEmail(String email) {
    return isMatch(email, DEFAULT_VALID_EMAIL_ADDRESS_REGEX);
  }

  public static boolean isValidPhone(String phoneNumber) {
    return isMatch(phoneNumber, DEFAULT_VALID_PHONE_NUMBER);
  }

  public static boolean isValidIpv4(String ip) {
    return isMatch(ip, DEFAULT_VALID_IPV4_REGEX);
  }

  public static boolean isValidIpv6(String ip) {
    return isMatch(ip, DEFAULT_VALID_IPV6_REGEX);
  }
  ...
  public static boolean isMatch(String input, String regex) {
    try {
      ...
      log.info("(isMatch)ERROR, input: {}, regex: {}", input, regex);
      return false;
    }
  }
  ...
  public static boolean isMatch(String input, Pattern pattern) {
    try {
      ...
      log.info("(isMatch)ERROR, input: {}", input);
      return false;
    }
  }
}
